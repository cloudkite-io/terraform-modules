...
resource "azurerm_key_vault_secret" "private_endpoint_connection_string" {
  ...
  value        = "Endpoint=sb://${azurerm_private_endpoint.private_endpoint[each.key].private_dns_zone_configs[0].record_sets[0].fqdn}/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=${azurerm_eventhub_namespace.events[each.key].default_primary_key}"
}
