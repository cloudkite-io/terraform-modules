...
| <a name="input_local_networks"></a> [local\_networks](#input\_local\_networks) | List of local virtual network connections to connect to gateway. | <pre>list(<br/>    object({<br/>      name                  = string<br/>      gateway_address       = string<br/>      address_space         = list(string)<br/>      shared_key            = string<br/>      ipsec_policy          = any<br/>      external_mappings_ref = optional(string, "")<br/>      ingress_nat_rule_ids  = optional(list(string), [])<br/>      egress_nat_rule_ids   = optional(list(string), [])<br/>    })<br/>  )</pre> | `[]` | no |
